...
const KNN = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${BACKEND_URL}/api/ml/predict/knn`,
        formData
      );
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0f1419 0%, #1a1f2e 100%)' }}>
      <Navigation title="K-Nearest Neighbors" />
      
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '2rem' }}>
        <div style={{ marginBottom: '2rem' }}>
          <h1 style={{ fontSize: '2.5rem', fontWeight: '700', color: '#e8eaed', marginBottom: '0.5rem' }}>
            Predicción de Churn con <span style={{ color: '#06a77d' }}>KNN</span>
          </h1>
          <p style={{ color: '#9ca3af', fontSize: '1.1rem', lineHeight: '1.6' }}>
            Utiliza el algoritmo de K-Nearest Neighbors para clasificar clientes según sus vecinos más cercanos.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ marginBottom: '2rem' }}>
          ...
          <div style={styles.card}>
            ...
            <div style={styles.grid}>
              ...
              <div className="form-group">
                ...
                <select name="MultipleLines" value={formData.MultipleLines} onChange={handleChange} className="form-select" data-testid="input-lines">
                  <option value="Yes">Sí</option>
                  <option value="No">No</option>
          ...
          <button
            ...
            disabled={loading}
            style={{ width: '100%', marginTop: '1rem', background: 'linear-gradient(135deg, #06a77d 0%, #048561 100%)' }}
